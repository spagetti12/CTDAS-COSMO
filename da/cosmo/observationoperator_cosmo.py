...
class ObservationOperator(object):
    ...
    def run(self, lag, dacycle, statevector):
        ...
        l_bg = np.empty(shape=self.forecast_nmembers)
       ...
       # LAMBDA FILE 1
        ...
        omem = ofile.createDimension('nensembles', self.forecast_nmembers/2)

        l = ofile.createVariable('lambda', np.float32, ('nensembles','nparam'),fill_value=-999.99)
        co2 = np.empty(shape=(int(self.forecast_nmembers/2),self.nparams))

        for m in range(0,int(self.forecast_nmembers/2)):
       ...
       # LAMBDA FILE 2
        ...
        opar = ofile.createDimension('nparam', self.nparams)
        omem = ofile.createDimension('nensembles', int(self.forecast_nmembers/2))

        l = ofile.createVariable('lambda', np.float32, ('nensembles','nparam'),fill_value=-999.99)
        co2 = np.empty(shape=(int(self.forecast_nmembers/2),self.nparams))

        for m in range(int(self.forecast_nmembers/2),self.forecast_nmembers):
            ...
            for row in csv_reader:
                for e in range(ensnum):
                    ...
                    dicts = np.append(dicts, {'ensnum':e, 'name':row[1], 'lon':row[2], 'lat':row[3], \
                                              ...
                                              'h':float(row[4]), 'deploy':float(row[5]), 'time':None, \
                                              'run':None, 'co2':[], 'extract':extract, 'extract_alt':None, \
    ...
    def get_hhl_data(self, dacycle, lag, cfile, dicts, starth, endh):
        ...
        hhl_bottom = np.empty(shape=(150, 225))
        ...
        for station in dicts:
            ...
            hhl_b = interpolate.interpn((rlat,rlon), hhl_bottom[:,:], [myrlat,myrlon], method='linear')

            if station['extract'] == 'abs':
                station['extract_alt'] = float(station['h']) + float(station['deploy'])
            elif station['extract'] == 'mid':
                station['extract_alt'] = float(station['deploy']) + hhl_b + (float(station['h']) - hhl_b)/2.
            elif station['extract'] == 'rel':
                station['extract_alt'] = hhl_b + float(station['deploy'])


            if float(station['extract_alt']) < hhl[59]:
                station['h1'] = hhl[59]
                station['h2'] = hhl[59]
                station['hidx1'] = 59
                station['hidx2'] = 59
                station['extract_alt'] = hhl[59]

            else:
                for l, ll in enumerate(hhl):
                    if float(station['extract_alt']) < ll:
                        station['h1'] = hhl[l]
                        station['h2'] = hhl[l+1]
                        station['hidx1'] = l
                        station['hidx2'] = l+1
...
#            logging.info('Reading of the files done for %s' % date_begin.strftime("%Y%m%d")+hrs)
            ...
            for station in dicts:
                ...
                co2_bg  = co2_bg001 * (l_bg[int(e)-1]/l_bg[0])
                ...
                if h1 == h2:
                    ...
                    co2_bg_final = co2_bg1 + (float(station['extract_alt'])-h1)*(co2_bg2-co2_bg1)/(h2-h1)
                    co2_bio_final = co2_bio1 + (float(station['extract_alt'])-h1)*(co2_bio2-co2_bio1)/(h2-h1)
                    co2_a_final = co2_a1 + (float(station['extract_alt'])-h1)*(co2_a2-co2_a1)/(h2-h1)
                    qv_final = qv1 + (float(station['extract_alt'])-h1)*(qv2-qv1)/(h2-h1)
    ...
    def write_cosmo_data(self, dacycle, dicts, date):
        ...
        for station in dicts:
            ...
            with Dataset(filename, mode='w') as ofile:
                ...
                olev[:] = float(station['extract_alt'])
...
#        os.system('python run_chain.py '+runname+' '+abs_start_time_ch+' '+starthour+' '+endhour+' -j meteo icbc')
        os.system('python run_chain.py '+runname+' '+abs_start_time_ch+' '+starthour+' '+endhour+' -j int2lm post_int2lm oae octe online_vprm cosmo -f')
