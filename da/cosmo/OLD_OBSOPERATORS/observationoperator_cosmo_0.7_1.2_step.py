...
class ObservationOperator(object):
    ...
    def run(self, lag, dacycle, statevector):
        ...
        l_bg = co2[:,-1]
        numlam = datetime.now().strftime("%H%M%S")
        os.system('cp '+self.lambda_file+' '+dacycle['da.vprm']+'/lambdas_'+numlam+'.nc')
        ...
        args = [
            (dacycle, dacycle['time.sample.start']+timedelta(hours = 24*n), dicts, rlat, rlon, path_in, l_bg)
    ...
    def get_cosmo_data(self, dacycle, date_begin, dicts, rlat, rlon, path_in, l_bg):
        hours = ['12', '13', '14', '15']
        ...
        for hrs in hours:
            ...
            ds = [cfgrib.open_datasets(fn+'_1')[0], cfgrib.open_datasets(fn+'_2')[0]]
#            logging.info('Reading of the files done for %s' % date_begin.strftime("%Y%m%d")+hrs)
            ...
            for station in dicts:
                ...
                co2_bio = ds[open_fn]['CO2_BIO'+e].values
