  ...
  #                                         self.ensemble_members[lag - 2][0].param_values 
   #         newmean = newmean / 3.0
        ...
        if lag == self.nlag - 1 and self.nlag >= 2:
            newmean += newmean + self.ensemble_members[lag - 1][0].param_values
            newmean = newmean / 3.0
            newmean[-1] = self.ensemble_members[lag - 1][0].param_values[-1]
...
#        sum_par = np.empty(self.nparams)
        ...
        for member in range(1, self.nmembers-3):
            rands = np.random.randn(self.nparams-1)
            rands_bg = np.random.normal(loc=0.0, scale=0, size=1)
            #rands_bg = np.random.normal(loc=0.0, scale=0.02, size=1)
...
#            randsC = np.hstack((np.dot(C, rands[0:int(self.nparams/2)]),np.dot(C, rands[int(self.nparams/2):self.nparams-1])))
            randsC = np.hstack((np.dot(C, rands[0:int(self.nparams/2)]),np.dot(C, rands[0:int(self.nparams/2)])))

            newmember.param_values = (np.hstack((randsC.flatten(),rands_bg)) + newmean).ravel()
#            newmember.param_values[newmember.param_values<0.] = 0. # NOT CUTTING OFF NEGATIVES!!!
            self.ensemble_members[lag].append(newmember)
#        self.ensemble_members[lag][0].param_values = sum_par/(self.nmembers-1)
#        print(np.mean(self.ensemble_members[lag][0].param_values))
## FOR OBS
        newmember = EnsembleMember(self.nmembers)
        newmember.param_values = np.hstack((0.7*np.ones(90),0.7*np.ones(90),1))
        self.ensemble_members[lag].append(newmember)

        newmember = EnsembleMember(self.nmembers)
        newmember.param_values = np.hstack((1.2*np.ones(90),1.2*np.ones(90),1))
        self.ensemble_members[lag].append(newmember)

        newmember = EnsembleMember(self.nmembers)
        newmember.param_values = np.hstack((1.4*np.ones(10),1.3*np.ones(10),1.2*np.ones(10),1.1*np.ones(10),1.0*np.ones(10),0.9*np.ones(10),0.8*np.ones(10),0.7*np.ones(10),0.6*np.ones(10),1.4*np.ones(10),1.3*np.ones(10),1.2*np.ones(10),1.1*np.ones(10),1.0*np.ones(10),0.9*np.ones(10),0.8*np.ones(10),0.7*np.ones(10),0.6*np.ones(10),1))
        self.ensemble_members[lag].append(newmember)
# FOR OBS DONE
...
#                nonegs[nonegs<0.] = 0.
  #              newmember.param_values = nonegs
    ...
    def write_members_for_cosmo(self, dacycle, advance, lag, outdir,endswith='.nc'):
...
# GPP
            if not advance:
                filename_gpp = os.path.join(outdir, 'parameters_gpp_'+str(lag)+'_'+dacycle['time.sample.stamp']+'.%03d%s' % (mem.membernumber, endswith))
            else:
                filename_gpp = os.path.join(outdir, 'parameters_gpp_'+str(lag)+'_'+dacycle['time.sample.stamp']+'_advanced.%03d%s' % (mem.membernumber, endswith))
...
# RESP
            if not advance:
                filename_resp = os.path.join(outdir, 'parameters_resp_'+str(lag)+'_'+dacycle['time.sample.stamp']+'.%03d%s' % (mem.membernumber, endswith))
            else:
                filename_resp = os.path.join(outdir, 'parameters_resp_'+str(lag)+'_'+dacycle['time.sample.stamp']+'_advanced.%03d%s' % (mem.membernumber, endswith))
