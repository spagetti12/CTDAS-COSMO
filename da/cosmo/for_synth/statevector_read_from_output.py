...
#        covariancematrix = np.identity(self.nparams)   # pavle - for testing, final matrix will be 23x23
        ...
        ifile = Dataset('/scratch/snx3000/parsenov/real/optimizer.20130401.nc', mode='r')
        newmean = ifile.variables['statevectormean_prior'][:]
        newmean = newmean.reshape(2,181)
#        newmean = #np.ones(self.nparams, float) # standard value for a new time step is 1.0
        ...
        newmember.param_values = newmean[lag,:].flatten()  # no deviations
        self.ensemble_members[lag].append(newmember)

        ifile = Dataset('/scratch/snx3000/parsenov/real/optimizer.20130401.nc', mode='r')
        par = ifile.variables['statevectordeviations_prior'][:]
        par = par.reshape(2,181,21)# + newmean
        for member in range(1, self.nmembers):
            newmember = EnsembleMember(member)
            newmember.param_values = par[lag, :, member] + newmean[lag,:]
            self.ensemble_members[lag].append(newmember)
