...
def analysis_pipeline(dacycle, platform, dasystem, samples, statevector):
    ...
    from da.stilt.expand_fluxes import save_weekly_avg_1x1_data, save_weekly_avg_state_data, save_weekly_avg_tc_data, save_weekly_avg_ext_tc_data, save_weekly_avg_agg_data
    from da.stilt.expand_molefractions import write_mole_fractions
...
def sample_step(dacycle, samples, statevector, obsoperator, lag, advance=False):
    ...
    for proc in range(nprocs):
        ...
        sampling_coords_file = os.path.join(dacycle['dir.input'], 'sample_coordinates_%s-%s.nc' %(dacycle['time.sample.stamp'],proc))
        samples.write_sample_coords(sampling_coords_file,proc,nprocs)
        ...
        p = mp.Process(target=obsoperator.run_forecast_model,args=(proc,out_q))
        procs.append(p)
    ...
    for proc in range(nprocs):
        if os.path.exists(sampling_coords_file):
            samples.add_simulations(resultdict[proc])
...
def advance(dacycle, samples, statevector, obsoperator):
    ...
    sampling_coords_file = os.path.join(dacycle['dir.input'], 'sample_coordinates_%s-0.nc' % dacycle['time.sample.stamp'])
    if os.path.exists(sampling_coords_file):
