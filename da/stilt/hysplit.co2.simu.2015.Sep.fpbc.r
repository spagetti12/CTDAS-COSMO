...
curdir="/Storage/CO2/wei/ctdas-hysplit-BCfp-randominit-alt3km-time/exec/da/stilt/"
...
# flog.info('eventids: %s',eventids[mm], name='logger.b')  #June 23 
  ...
  {			
    ...
    endpts[,5]=replace(endpts[,5],is.na(endpts[,5]),0)    #NA existed in height, this bug is fixed Oct 21, 2015
    endpts[,6]=replace(endpts[,6],is.na(endpts[,6]),0)
...
##    ixflux=(1:ntimes)[(datevalid<=xx)][n:(endtime+n-1)]
    ...
    if(boundary_flag == "CTNA" || boundary_flag == "CTE")
    {
      ...
      flog.info("npts = %f",npts,name='logger.b')
        ...
        if (lonarr[p] < -125 || lonarr[p] > -55 || latarr[p] < 20 || latarr[p] > 75)
         {
            ...
            if (k0>3000)
            {
                ...
                t_weight = t_weight +  (1-diff/(24*ndays))/npts
    ...
    flog.info('FP_BC_d based on altitude: %s',FP_BC_c*t_weight,name='logger.b')
...
#{
    ...
    ncatt_put( ncid_new, 0, "Source", "CTDAS-HYSPLIT-NAM12 v1.0 fluxes, generated from SiB3, SiBCASA and CT2013B products")      
