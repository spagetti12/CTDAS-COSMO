...
#inputs and outputs
stiltfile="stilt.rc"
curdir="/Storage/CO2/wei/ctdas-stilt-forward/exec/da/stilt/"
...
for(mm in 1:length(umon))  
{
  ...
  #stilt2010x01x01x18x59x33.4057Nx081.8334Wx00305x289079.nc
  #hysplit_2010x01x05x19x45.95Nx090.27Wx00396x289916_footprint.nc
...
for(mm in 1:length(pfbfns))  
{
  ...
  if(length(foot)>100)  #2010x01x05x19x45.95Nx090.27Wx00396x289916
  {			
   ...
   #bioflux[,,]=replace(bioflux[,,],is.na(bioflux[,,]),0)
    ...
    for (i in 1:nsam) 
    {
      if(xxleft<b2)  # && xxleft>=b1  #flux with scaling factors
      {    
        diff=abs(as.numeric(difftime(b2,xxleft,units='hours')))  # needs scaling for first half
       ...
       #     diff=1
        ...
        for (hh in 1:(diff)) 
        {
         ...
         # else
         # {
          #   log=paste("ERROR activated!!hh=%d,ntimes1=%d", hh, ntimes1,sep="")
          #   flog.error(log, name='logger.b')
         # }
        }

        for (hh in (diff+1):ntimes1)
        {
           if(hh<=ntimes1 && hh>=diff+1)
