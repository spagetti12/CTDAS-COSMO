...
#        self.nparams = 23
    ...
    def create_matrices(self):
        ...
        self.x_out = np.zeros((self.nlag * (self.nparams+2),), float)
        # deviations from mean state  [X']
        self.X_prime = np.zeros((self.nlag * self.nparams, self.nmembers,), float)
        self.X_prime_out = np.zeros((self.nlag * (self.nparams+2), self.nmembers,), float)
    ...
    def state_to_matrix(self, statevector):
        ...
        for n in range(self.nlag):
            ...
            self.x[n * self.nparams:(n + 1) * self.nparams] = np.hstack([members[0].param_values[0:10],members[0].param_values[11:21],members[0].param_values[22]])
            self.X_prime[n * self.nparams:(n + 1) * self.nparams, :] = np.transpose(np.array([np.hstack([m.param_values[0:10],m.param_values[11:21],m.param_values[22]]) for m in members]))
    ...
    def matrix_to_state(self, statevector):
        for n in range(self.nlag):
            ...
            for m, mem in enumerate(members):
                temporary_var = self.X_prime[n * self.nparams:(n + 1) * self.nparams, m] + self.x[n * self.nparams:(n + 1) * self.nparams]
                members[m].param_values[:] = np.hstack([temporary_var[0:10],0,temporary_var[10:20],0,temporary_var[20]])
...
#        savedict['values'] = self.x.tolist()
        ...
        if type == 'prior':
            ...
            print('self.X_prime.shape AAA',self.X_prime.shape)
        elif type == 'optimized':
            savedict['values'] = self.X_prime_out.tolist()
            print(self.X_prime_out.shape)
...
#WP      should always be updated last because it features in the loop of the adjustments !!!!
            ...
            # added by pavle
            for r in range(self.nmembers):
                self.X_prime_out[:, r] = np.hstack([self.X_prime[:, r][0:10],0,self.X_prime[:, r][10:20],0,self.X_prime[:, r][20],self.X_prime[:, r][21:31],0,self.X_prime[:, r][31:41],0,self.X_prime[:, r][41]])
        self.x_out[:] = np.hstack([self.x[:][0:10],0,self.x[:][10:20],0,self.x[:][20],self.x[:][21:31],0,self.x[:][31:41],0,self.x[:][41]])
