...
#                obspackid = 'jbga_bzhe_'+str(obspacknum)
#                obspackid = str(obspacknum).split(" ")
#                obspackid = ['jbga_bzhe_'+s for s in split(str(obspacknum)," ")]
                obspackid = []
                for o in obspacknum:
                    obspackid = append(obspackid,'jbga_bzhe_'+str(o))
#                obspackid = ' '.join(str(obspacknum))
#            print obspackid
#            sys.exit()
...
#            obspackid = map(strip, obspackid)
            ...
            lats_1d = ncf.get_variable('latitude')#.take(subselect, axis=0)
            lons_1d = ncf.get_variable('longitude')#.take(subselect, axis=0)
            alts_1d = ncf.get_variable('altitude')#.take(subselect, axis=0)

            lats = empty(shape=(len(obspacknum)))
            lons = empty(shape=(len(obspacknum)))
            alts = empty(shape=(len(obspacknum)))
            print(lats_1d.shape)
            print(lats.shape)
#            sys.exit()
            lats[:] = lats_1d
...
#                print type(obspacknum)
 ...
 #               print type(lats)
 #               print type(lons)
 #               print type(obspackid)

#                print obspacknum.shape
#                print dates.shape
...
#                print 'PAVLE   MISSING SAMPLES ',idx,'val ',val,'ids ',ids,'simulated ',simulated
...
#                f.add_attribute(key, msg)
            ...
            savedict['dims'] = dimid
            savedict['values'] = data.tolist()
#            print 'OBS DATA?????',data.tolist()      # ABSOULTE DATA IN MOL/MOL
            ...
            f.add_data(savedict)
#            print 'pavle start ',savedict,' pavle end'
#            sys.exit()
            f.close()
 ...
 #               identifier, incalt = value.split(';')
  #              site_incalt[identifier.strip()] = (int(incalt))
        ...
        for obs in self.datalist:  # first loop over all available data points to set flags correctly
            obs.mdm = 1e-6  #  PAVLE - the mdm for ALL sites is 2.5 (2.5e-6) ppm, which corresponds to "land" site
#            obs.mdm = 2.5e-6  #  PAVLE - the mdm for ALL sites is 2.5 (2.5e-6) ppm, which corresponds to "land" site
...
#            species, site, method, lab, datasetnr = identifier.split('_')
            #    identifier is filename w/o .nc / e.g. co2_fake_obs_lhw_ok
            fake, ooo, site, ok = identifier.split('_')
#            co2, fake, ooo, site, ok = identifier.split('_')
#            print 'PAVLE        OBS ',co2, fake, ooo, site, ok
#            print obs.code,obs.mdm

#            if site_info.has_key(identifier):                  # DOES NOT GO HERE
...
#        print 'BOUTSA SITE',self.site_info,self.site_move,self.site_incalt
#        sys.exit()
...
#            f.add_attribute(key, msg)
        ...
        savedict['values'] = data.tolist()        # in mol/mol
...
class MoleFractionSample(object):
    ...
    def __init__(self, idx, xdate, code='XXX', obs=0.0, simulated=0.0, resid=0.0, hphr=0.0, mdm=0.0, flag=0, height=0.0, lat= -999., lon= -999., evn='0000', species='co2', samplingstrategy=1, sdev=0.0, fromfile='none.nc'):
        ...
        self.may_reject = False # PAVLE EDIT              # Whether sample may be rejected if outside threshold
